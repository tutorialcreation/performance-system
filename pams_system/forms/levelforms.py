...
class LevelStructureForm(BSModalForm, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['maplist'].queryset = InputData.objects.none()
        ...
        if 'maptype' in self.data:
            try:
                ...
                self.fields['maplist'].queryset = InputData.objects.filter(maptype_id=maptype_id)
            except (ValueError, TypeError):
                pass
        elif self.instance.pk:
            self.fields['maplist'].queryset = InputData.objects.filter(maptype_id=2)

        if 'maplist' in self.data:
            try:
                maplist_id = int(self.data.get('maplist'))
                self.fields['levelset'].queryset = InputData.objects.filter(maplist_id=maplist_id)
            except (ValueError, TypeError):
                pass  # invalid input from the client; ignore and fallback to empty City queryset
        elif self.instance.maplist_id:
            self.fields['levelset'].queryset = InputData.objects.filter(maplist_id=1)

        if 'parent' in self.data:
            try:
                levelset_id = int(self.data.get('parent'))
                self.fields['parent'].queryset = InputData.objects.filter(levelset_id=levelset_id)
            except (ValueError, TypeError):
                pass
        elif self.instance.pk:
...
class KpiStructureForm(BSModalForm, forms.ModelForm, forms.Form):
    class Meta:
        model = InputData
        fields = ['maptype', 'maplist', 'levelset', 'parent', 'kpis']
        widgets = {
            'parent': Select2MultipleWidget,
        }
